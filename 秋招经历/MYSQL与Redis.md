...
创建一个（a,b）的联合索引， **a在等值的情况下，b值又是按顺序排列的，但是这种顺序是相对的。这是因为MySQL创建联合索引的规则是首先会对联合索引的最左边第一个字段排序，在第一个字段的排序基础上，然后在对第二个字段进行排序。所以b=2这种查询条件没有办法利用索引**。 

```sql
select * from staffs where id like 'A%';//前缀都是排好序的，使用的都是联合索引
select * from staffs where id like '%A%';//全表查询
select * from staffs where id like '%A';//全表查询
select * from staffs where id > 2 and id < 5 and age >18;//不遵循最左匹配原则，但在数据库中id<2的只有一条（id），所以在id>2 && id<5的范围中，age是有序的，所以使用的是rang范围查询。
```
...
+ 如果sql为 `SELECT * FROM table WHERE a = 1 and b = 2 and c = 3; `如何建立索引?

  > (a,b,c)或者(c,b,a)或者(b,a,c)都可以建立索引，重点要的是将区分度高的字段放在前面，区分度低的字段放后面。像性别、状态这种字段区分度就很低，我们一般放后面。

+ 如果sql为 `SELECT * FROM table WHERE a > 1 and b = 2;  `如何建立索引?

  > 对(b,a)建立索引。如果建立的是(a,b)索引，那么只有a字段能用得上索引，毕竟最左匹配原则遇到范围查询就停止匹配。
...
+ 如果sql为 `SELECT * FROM table WHERE a = 1 ORDER BY b;`如何建立索引?
...
+ 如果sql为 `SELECT * FROM table WHERE a IN (1,2,3) and b > 1;  `如何建立索引?
...
+ ==**跳跃表**==
  ...
  + 如果有序集合包含的元素数量较多，或者有序集合中元素的成员是比较长的字符串时，Redis使用跳跃表来实现有序集合键。
...
+ Sorted Set
  ...
  + 举例: 在播系统中，实时排行信息包含直播间在线用户列表，各种礼物排行榜，弹幕消息(可以理解为按消息维 度的消息排行榜)等信息，适合使用 Redis 中的 SortedSet 结构进行存储。

---

##### zset是如何实现的

有序列表。兼具了Hash集合和Set的双重特性，保证了value值的唯一性的同时又保证了高性能，最主要的是还可以给每个Value设置Source(权重)，可以通过权重进行排序。
