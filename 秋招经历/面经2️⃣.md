...
+ 显示类型转换：
  + 旧有的C风格的强制类型转换。这些不推荐，在代码中不显眼，容易被忽略， 而且实际上是困难且容易出错的。
  + explicit关键字：声明为explicit的构造函数不能在隐式转换中使用，但只支持一个参数的构造函数
  + 四种强制类型转换
...
+ 原理：智能指针是一个类，这个类的构造函数中传入一个普通指针，析构函数中释放传入的指针。智能指针的类是栈上的对象，智能指针指向堆上开辟的空间，函数结束时，栈上的函数会自动被释放，智能指针指向的内存也会随之消失，防止内存泄漏。
...
+ `std::condition_variable`可以和`std::mutex`结合一起使用，其中有两个重要的接口，`notify_one()`和`wait()`
  + `wait()`可以让线程陷入**休眠状态**
