...
+ 可以使用C/C++语言进行开发，熟悉C/C++基本语法及其编程思想
  - 熟悉C++的封装类，函数重载原理，多态机制，强制类型转换以及C++11新增语法
  - 了解静态与动态链接库
  - 了解STL，能够使用STL部分容器，例如vector, list, map等
+ 了解操作系统相关知识，例如进程，线程，虚拟内存等
+ 了解计算机网络相关知识，例如OSI七层模型与TCP/IP协议栈，以及TCP/UDP协议，HTTP/HTTPS等
+ 熟悉Socket编程，熟悉系统编程技术，多进程和多线程编程等
+ 掌握常见数据结构，例如单链表，双向链表，栈和队列，二叉树等
+ 掌握基础排序算法，例如选择，冒泡，快排，归并，堆排序等

---

##### **介绍一下你的WebServer项目**

+ 自己用C++11改写了一个简单的Reactor高并发模型服务器。主线程初始化一个线程池，然后建立socket链接，进行绑定IP与端口，创建socketfd，利用IO复用技术Epoll模型efd，将socketfd添加至efd中，进行监听事件与数据读写。
+ 定义httpconn类封装任务事件，包括socket文件描述符，地址信息，读写缓存区已经HTTP相关内容；
+ 如果监听到连接事件发生，则进行accpet连接，新建socket文件描述符 connfd ，然后将连接的用户数据放入 users 数组中并初始化，顺便添加至epoll中进行监听。
+ 如果有写事件出发，则将读数据的操作交给线程池中的线程将对应connfd的数据一次性读，读入读缓冲区
+ 数据读完之后，将任务封装成 http_conn 类，线程池中子线程取出一个任务，执行任务中的process函数，即解析HTTP请求，生成响应写入写缓冲区，触发可写操作。
+ 监听到可写操作，主线程让子线程去完成写操作，将写写缓冲区的内容写给客户端
