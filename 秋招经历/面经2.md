...
<img src="../图片/STL迭代器失效.png" width="800px" />
...
<img src="../图片/STL_deque.png" width="500px" />
...
##### 15. **map、set是怎么实现的，红黑树是怎么能够同时实现这两种容器? 为 什么使用红黑树?**
...
+ leftleft情况：节点是其父亲的左儿子，其父亲又是上一节点的左儿子
  ...
  +  **进行右旋：右手向下扳祖节点，让/变成⋀，自己变为左孩子的右孩子；**
    ...
    <img src="../图片/AVL左单旋转.png" width="600px" />
...
+ leftright情况 
  ...
  + 先做左旋转，形成leftleft情况，再做右旋转
    ...
    <img src="../图片/AVL左右旋转1.png" width="400px" /><img src="../图片/AVL左右旋转2.png" width="300px" />
...
+ rightleft旋转 
  ...
  <img src="../图片/AVL右左旋转1.png" width="300px" /><img src="../图片/AVL右左旋转2.png" width="250px" />
...
+ 面试官您好，我叫张永斌，是一名来自东南大学的网安学院的在读研究生。专业方向是计算机专业。目前的科研方向是5G无线安全算法，研究目的是基于物理层安全，实现可靠高效的5G无线通信安全。在科研项目工程上我主要负责5G小基站项目相关平台开发与维护。

  除了科研外，对后端开发比较感兴趣，学习了计算机相关和后端开发相关的知识，并且尝试了一些小项目进行实践，辅助学习；个人希望未来能够从事互联网开发相关的工作，实现兴趣与工作的相统一。

  个人对待学习和工作认真负责，具备独立思考、分析解决问题的能力，于此同时也积极保持与实验室老师，同学相互沟通，积极融入团队。

  最后，我是一个性格开朗，热爱生活的人，有较强的适应与抗压能力，平时喜欢跑步，跳绳，改善身体状态。

  以上是我的一个简单自我介绍，感谢您的聆听。
...
+ C++11新的方法需要进行单独的学习，例如，之前线程我用 pthread_create 创建，现在 利用 thread 创建，还有一些锁的写法，给编程带来方便的同时，也需要进行学习。还有一些 lambda 函数，move 的使用。
+ 主要解决方法就是自己查阅资料，比如C++并发编程那本书对于C++11中的多线程阐述的很好，可惜的是因为时间的关系，只将自己需要的部分学习完成了，还没拜读完整本书籍，等秋招结束后肯定补回来。
+ 以及和实验室的同学进行沟通讨论，因为他们也对C++后端很感兴趣，所以也写了这个项目，当出现问题的时候大家来讨论时，能够找到很多新奇的想法与思路，比如设计的实现自动增长的缓冲区；

- 多线程相关的处理时，临界区的访问，互斥量的使用需要深思熟虑，如果没有考虑好，很容易造成死锁；最后是通过自己先写案例测试，然后再添加到项目中。
- 处理 http 请求，解析时候的状态转换，逻辑问题。

---

##### 如何改善自己写的服务器？

+ 自己写的服务器是在租的华为云服务器上的，因为便宜所以买的最低配，内存1G，单核。所以想要提高性能，首先应该改善硬件，使用多核的配置。这样才能充分发挥多线程的优点。
...
+ 目前现有的通信安全机制是基于上层的加密体制以及相关的加密协议，例如公钥加密对称密钥，证书等等。这种体制有种弊端：

1. 基于计算复杂度，随着量子计算机的发展，敌方的计算能力大幅度提高，可能会直接破解密钥；
2. 密钥分发管理涉及到一整套的协议，比较繁琐。而无线物理层通信安全利用通信双方之间的物理信道信息，提取密钥，在通信过程中完成加密，将原来的通信与安全融合到一起，并且可以逼近实现一次一密。

##### 2.介绍一下个人研究

和传统的高层认证不同，物理层认证从通信本身的信号格式、无线设备自身的特性及无线信道“内生”的特征出发致力于 实现一种信息论上的安全;

由于舍弃了高层繁琐的加密解密操作，因此可节省认证时延及计算开销。根据其利用的参数、实现的方法等不同，物理层认证的研究主要可分为物理层水印、设备射频指纹、无线信道指纹、物理层挑战-响应、跨层认证五个方面。

本人的研究主要着重与在5G协议中融合无线信道指纹，实现跨层认证。

现有的认证机制在物理层实现认证，不需要像高层认证那样进行繁琐的加密解密工作，有效的降低了系统开销和延迟。但是，此类方法均假设攻击方无法获取合法双方的信道信息，其安全性强烈依赖无线信道的私密性。但在实际通信环境中，一旦攻击方获得合法信道信息，便可直接根据认证响应破解密钥，认证存在着密钥泄露的安全隐患。

所以本人提出了一种联合密钥的方法。将信道指纹信息与双方共有的密钥形成 $2 * n$ 维的密钥矩阵，利用插值多项式算法生成多项式方程曲线，仅经历相同信道且具有相同密钥的双方才能产生相同的认证响应。这样的话

1. 若双方的利用插值多项式生产的方程曲线的差别足够小，则得到的认证响应差别也将足够小
...
+ 一些代码的优化
  ...
  + 因为我们的项目基于Intel体系架构，而且是利用icc进行编译，所以可以将代码中的一些运算利用avx指令集进行改写，利用它多流水线处理的特征提升效率

    >  其寄存器宽度再次加倍，来到512-bit，能够处理更多的数据

  + 比如两个向量进行相加，分别是1\*4维度的，一般逻辑是利用for循环进行处理，而利用`_mm256_set_epi64x` 指令就能创建一个1\*4维度的，每个数为64位的向量，然后利用指令`_mm256_add_epi64`就能直接相加得出结果，提高了效率。
...
+ ICC全称Intel C++ Compiler，是intel 针对intel 体系架构开发的编译器，适用于Linux、Microsoft和Mac OS X操作系统。我们项目是运行在intel机器上，使用编译出来的可执行代码效率更高。
...
+ GCC:GNU Compiler Collection(GUN 编译器集合)，它可以编译C、C++，Object-C等语言
